    ...
    def forward(self, audio_features, text_features, logit_scale_a, logit_scale_t=None, audio_features_mlp=None, text_features_mlp=None):
        ...
        if self.mlp_loss:
            if self.world_size > 1:
                ...
                if self.local_loss:
                    logits_per_audio = logit_scale_a * audio_features_mlp @ all_text_features_mlp.T
                    logits_per_text = logit_scale_a * text_features_mlp @ all_audio_features_mlp.T
                    ...
                    logits_per_audio = logit_scale_a * all_audio_features_mlp @ all_text_features_mlp.T
                ...
                logits_per_audio = logit_scale_a * audio_features_mlp @ text_features_mlp.T
                logits_per_text = logit_scale_a * text_features_mlp @ audio_features_mlp.T
            ...
            if not self.weighted_loss:
                ...
                audio_weight = (audio_features_mlp@audio_features_mlp.T).detach()
                audio_weight = (torch.exp(torch.sum(audio_weight, axis=1)/(self.weight_loss_kappa*len(audio_weight)))).detach()
                text_weight = (text_features_mlp@text_features_mlp.T).detach()
...
class FeaturePredictionLoss(nn.Module):
    ...
    def forward(self, feat_gt, feat_pred_audio, feat_pred_txt):
        feature_prediction_loss ={
            "feature_prediction_loss_audio": sum([self.mseloss(feat_gt[k], feat_pred_audio[k]) for k in feat_gt.keys()]),
            "feature_prediction_loss_txt": sum([self.mseloss(feat_gt[k], feat_pred_txt[k]) for k in feat_gt.keys()]),
            "feature_prediction_loss_cross": sum([self.mseloss(feat_pred_audio[k], feat_pred_txt[k]) for k in feat_gt.keys()])
        }
        return feature_prediction_loss