...
roberta_tokenizer = AutoTokenizer.from_pretrained("rinna/japanese-roberta-base")
        ...
        if len(audio_data) > max_len:
            ...
            if len(audio_data) < max_len:  # do nothing if equal
                if data_filling == "repeatpad":
                    if len(audio_data) == 0: print(sample)
...
def preprocess_single(
    ...
    audio_data = float32_to_int16_torch(audio_data[0]).to(torch.float32)
    ...
    if class_index_dict is not None:
        ...
        sample["class_label"] = torch.tensor(class_labels).to(torch.float32)

    del sample[text_ext]
    sample["audio_name"] = sample["__key__"].split("/")[-1] + "." + audio_ext
    sample["text_name"] = sample["__key__"].split("/")[-1] + "." + text_ext
    sample["feat_name"] = sample["__key__"].split("/")[-1] + "." + text_ext
    sample["audio_orig_sr"] = orig_sr

    sample["pitch_mean"] = (sample[feat_ext]["pitch"] - stat_dict["pitch"]["mean"]) / stat_dict["pitch"]["std"]
    sample["energy_std"] = (sample[feat_ext]["energy"] - stat_dict["energy"]["mean"]) / stat_dict["energy"]["std"]
    sample["mps"] = (sample[feat_ext]["mps"] - stat_dict["mps"]["mean"]) / stat_dict["mps"]["std"]
...
def collate_fn_with_preprocess(batch,
    ...
    for sample in batch:
        data_preprocessed.append(
            preprocess_single(sample, audio_ext, text_ext, feat_ext, stat_dict, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
    ...
    for k in data_preprocessed[0].keys():
        if isinstance(data_preprocessed[0][k], dict):  # dealwith bert tokenizer output
            ...
            batch_dict[k] = feature_extractor([sample[k].view(-1).to('cpu').detach().numpy().copy() for sample in data_preprocessed], padding=True, return_attention_mask=True, sampling_rate=16000)
            for label, value in batch_dict[k].items():
                if label == "input_values":
                    value = [float32_to_int16(v).astype(np.float32) for v in value]
                batch_dict[k][label] = torch.tensor(np.stack(value))
            ...
            batch_dict[k] = torch.tensor(np.stack([sample[k] for sample in data_preprocessed]), dtype=torch.float32)
        else:
...
def get_wds_dataset(
    ...
    if not statfilepath_ is None:
        statfilepath = statfilepath_
    else:
        statfilepath = os.path.join(os.path.dirname(input_shards[0]), os.pardir, "stat.json")
