...
class CLAP_Module(torch.nn.Module):
    def __init__(self, enable_fusion=False, device=None, amodel= 'HTSAT-tiny', tmodel='roberta') -> None:
        ...
        self.tokenize = AutoTokenizer.from_pretrained('rinna/japanese-roberta-base')
    ...
    def get_audio_embedding_from_data(self, x, use_tensor=False):
        ...
        for audio_waveform in x:          
            # quantize
            if not use_tensor:
                if audio_waveform.dtype == np.dtype("int16"):
                    audio_waveform = float32_to_int16(int16_to_float32(audio_waveform))
                audio_waveform = torch.from_numpy(audio_waveform).float()
            temp_dict = {}
            temp_dict = get_audio_features(
                temp_dict, audio_waveform, 160000, 
                data_truncating='fusion' if self.enable_fusion else 'rand_trunc', 
                data_filling='pad',
                audio_cfg=self.model_cfg['audio_cfg'],
                require_grad=audio_waveform.requires_grad
            )
            temp_dict["waveform"] = feature_extractor(temp_dict["waveform"].to('cpu').detach().numpy().copy(), padding=True, return_attention_mask=True, sampling_rate=16000)
            temp_dict["waveform_input_values"] = torch.tensor(float32_to_int16(temp_dict["waveform"]["input_values"][0]).astype(np.float32))
            temp_dict["waveform_attention_mask"] = torch.tensor(temp_dict["waveform"]["attention_mask"][0])
            del temp_dict["waveform"]
            audio_input.append(temp_dict)
        audio_embed = self.model.get_audio_embedding(audio_input)
        if not use_tensor:
            audio_embed = audio_embed.detach().cpu().numpy()
    ...
    def get_text_embedding(self, x, tokenizer = None, use_tensor = False):
        ...
        text_input = {k: v.view(1, -1) if v.dim()<2 else v for (k,v) in text_input.items()}
